...
public class KalApiTokenFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (StringUtils.isEmpty(authToken)) {
            RequestUtil.sendErrorToResponse(ErrorCode.UNAUTHORIZED_SIGN.apply("No token provided"), (HttpServletResponse) response);
            return;
        }
        ...
        try {
            ...
            RequestUtil.sendErrorToResponse(ErrorCode.UNAUTHORIZED_SIGN.apply("Unable to check token validity"), (HttpServletResponse) response);
            return;
        }
        ...
        try {
            ...
            RequestUtil.sendErrorToResponse(ErrorCode.UNAUTHORIZED_SIGN.apply("Unable to check if current token gives access to API"), (HttpServletResponse) response);
            return;
        }

        if (validateRoleURIResponse.getStatusCode().value() != OK.value()) {
            RequestUtil.sendErrorToResponse(ErrorCode.UNAUTHORIZED_SIGN.apply("Provided token doesn't give access to API"), (HttpServletResponse) response);
            return;
        }
