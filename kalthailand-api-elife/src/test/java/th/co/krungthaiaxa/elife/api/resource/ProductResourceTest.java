...
public class ProductResourceTest {
    ...
    public void should_return_amounts_and_common_data() throws IOException {
        ...
        ResponseEntity<String> response = template.exchange(builder.toUriString(), PUT, new HttpEntity<>(productQuotation(30, EVERY_MONTH)), String.class);
        assertThat(response.getStatusCode().value()).isEqualTo(OK.value());
        ProductAmounts productAmounts = getProductAmountsFromJSon(response.getBody());
    ...
    public void should_return_common_data_only() throws IOException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromUri(base);
        ResponseEntity<String> response = template.exchange(builder.toUriString(), PUT, new HttpEntity<>(productQuotation(30, null)), String.class);
        assertThat(response.getStatusCode().value()).isEqualTo(OK.value());
        ProductAmounts productAmounts = getProductAmountsFromJSon(response.getBody());
        assertThat(productAmounts.getCommonData()).isNotNull();
