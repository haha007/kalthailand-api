...
public class PolicyDocumentService {
    ...
    @Autowired
    public PolicyDocumentService(DocumentService documentService,
    ...
    public void generateDocumentsForPendingValidationPolicy(Policy policy, final String accessToken) {
        ...
        if (!notValidatedApplicationForm.isPresent()) {
            try {
                ...
                byte[] signedPdf = signingDocumentService.signPDFFile(content, accessToken);

                documentService.addDocument(policy, signedPdf, "application/pdf", APPLICATION_FORM);
            } catch (Exception e) {
        ...
        if (ProductUtils.isAtpModeEnable(policy)) {
            ...
            if (!daForm.isPresent()) {
                try {
                    ...
                    documentService.addDocument(policy, signedPdf, "application/pdf", DA_FORM);
                } catch (Exception e) {
    ...
    public void generateDocumentsForValidatedPolicy(Policy policy, final String accessToken) {
        ...
        generateDocumentsForPendingValidationPolicy(policy, accessToken);
        ...
        if (!validatedApplicationForm.isPresent()) {
            try {
                ...
                documentService.addDocument(policy, signedPdf, "application/pdf", APPLICATION_FORM_VALIDATED);
        ...
        if (!ereceiptPdf.isPresent()) {
            ereceiptService.addEReceiptPdf(policy, firstPayment, newBusiness, accessToken);
        } else {
